...
export default function Home(){
  ...
  const P10_SLIDES=["./p10-slide-1.png","./p10-slide-2.png","./p10-slide-3.png","./p10-slide-4.png","./p10-slide-5.png","./p10-slide-6.png","./p10-slide-7.png"];
  ...
  if(screen==="blank1"&&profile)return <main className="questIntroScreen" style={{overflowY:"auto"}}><header className="missionNav missionNavTrust"><button className="brand brandButton" onClick={reset}><span className="brandMark">e·</span><span>Envizi<br/>Impact Quest</span></button><div className="missionProgress"><span className="activeDot"/> IL QUEST</div>{renderTrustBar()}<button className="langMini" onClick={()=>setLanguage(language==="it"?"en":"it")}>{language==="it"?"EN":"IT"}</button></header><section style={{display:"flex",flexDirection:"column",alignItems:"center",gap:"32px",padding:"32px 24px",width:"100%"}}><img src="./slide-education-1.png" alt="Education slide 1" style={{width:"80%",maxWidth:"80%",borderRadius:"8px",boxShadow:"0 4px 24px rgba(0,0,0,.45)"}}/><div style={{display:"flex",gap:"16px",flexWrap:"wrap",justifyContent:"center"}}><button className="introBackBtn" onClick={()=>setScreen("questIntro")}>← {language==="it"?"Indietro":"Back"}</button><button className="actionButton" style={{background:"#1a56db",color:"#fff",border:"none",borderRadius:"6px",padding:"10px 22px",fontWeight:700,cursor:"pointer",fontSize:"15px"}} onClick={()=>{setP10SlideIdx(0);setScreen("p10Slideshow");}}>{language==="it"?"▶ Avvia presentazione":"▶ Start slideshow"}</button><button className="actionButton approachIntroCta" onClick={()=>setScreen("blank2")}>{language==="it"?"Avanti":"Next"}<b>→</b></button></div></section></main>;

  if(screen==="p10Slideshow"&&profile)return <main style={{background:"#000",display:"grid",gridTemplateRows:"auto 1fr auto",height:"100dvh",overflow:"hidden"}}><header style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 20px",background:"rgba(0,0,0,.85)",zIndex:10}}><button className="brand brandButton" onClick={reset} style={{color:"#fff"}}><span className="brandMark">e·</span><span>Envizi<br/>Impact Quest</span></button><span style={{color:"#aaa",fontSize:"13px"}}>{p10SlideIdx+1} / {P10_SLIDES.length}</span><button className="langMini" onClick={()=>setScreen("blank1")} style={{background:"transparent",border:"1px solid #555",color:"#ccc",borderRadius:"4px",padding:"4px 10px",cursor:"pointer"}}>✕ {language==="it"?"Chiudi":"Close"}</button></header><section style={{display:"flex",alignItems:"center",justifyContent:"center",overflow:"hidden",background:"#000"}}><img src={P10_SLIDES[p10SlideIdx]} alt={`Slide ${p10SlideIdx+1}`} style={{maxWidth:"100%",maxHeight:"100%",objectFit:"contain"}}/></section><footer style={{display:"flex",alignItems:"center",justifyContent:"center",gap:"24px",padding:"12px 20px",background:"rgba(0,0,0,.85)"}}><button onClick={()=>setP10SlideIdx(i=>Math.max(0,i-1))} disabled={p10SlideIdx===0} style={{background:"transparent",border:"1px solid #555",color:p10SlideIdx===0?"#444":"#ccc",borderRadius:"6px",padding:"8px 20px",cursor:p10SlideIdx===0?"not-allowed":"pointer",fontSize:"15px"}}>←</button><div style={{display:"flex",gap:"6px"}}>{P10_SLIDES.map((_,i)=><span key={i} onClick={()=>setP10SlideIdx(i)} style={{width:"8px",height:"8px",borderRadius:"50%",background:i===p10SlideIdx?"#39efb4":"#555",cursor:"pointer",display:"inline-block"}}/>)}</div><button onClick={()=>p10SlideIdx===P10_SLIDES.length-1?setScreen("blank1"):setP10SlideIdx(i=>i+1)} style={{background:p10SlideIdx===P10_SLIDES.length-1?"#1a56db":"transparent",border:"1px solid #555",color:"#ccc",borderRadius:"6px",padding:"8px 20px",cursor:"pointer",fontSize:"15px"}}>{p10SlideIdx===P10_SLIDES.length-1?(language==="it"?"✓ Fine":"✓ Done"):"→"}</button></footer></main>;
