    ...
    <main style={{display:"flex",flexDirection:"column",minHeight:"100dvh",background:"var(--bg)",overflowY:"auto"}}>
      ...
      <section style={{display:"flex",flexDirection:"column",alignItems:"center",padding:"40px 24px 60px",gap:"32px",width:"100%",boxSizing:"border-box"}}>
        <div style={{textAlign:"center",maxWidth:"680px"}}>
          <small style={{letterSpacing:".18em",fontSize:"11px",color:"var(--muted)",textTransform:"uppercase",fontWeight:700}}>{language==="it"?"ENVIZI QUEST · MAPPA DEI CAPITOLI":"ENVIZI QUEST · CHAPTER MAP"}</small>
          <h1 style={{fontSize:"clamp(24px,4vw,40px)",fontWeight:800,margin:"12px 0 8px",lineHeight:1.15}}>{language==="it"?"L'Esperienza Envizi Quest":"The Envizi Quest Experience"}</h1>
          <p style={{color:"var(--muted)",fontSize:"15px",lineHeight:1.6,margin:0}}>{language==="it"?"Salta direttamente al capitolo che ti interessa o procedi dall'inizio.":"Jump directly to the chapter you want or proceed from the beginning."}</p>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(260px,1fr))",gap:"14px",width:"100%",maxWidth:"960px"}}>
          {CHAPTER_MAP_ITEMS.map((ch)=>(
            <button key={ch.screen} onClick={()=>setScreen(ch.screen)} style={{display:"flex",alignItems:"center",gap:"16px",background:"var(--surface,#1a1a2e)",border:"1px solid var(--border,rgba(255,255,255,.1))",borderRadius:"12px",padding:"16px 20px",cursor:"pointer",textAlign:"left",transition:"border-color .15s",color:"inherit"}}
              ...
              <span style={{minWidth:"36px",height:"36px",borderRadius:"8px",background:"var(--accent,#39efb4)",color:"#000",fontWeight:800,fontSize:"12px",display:"flex",alignItems:"center",justifyContent:"center",letterSpacing:".04em",flexShrink:0}}>{ch.icon}</span>
              <span style={{fontWeight:600,fontSize:"14px",lineHeight:1.35}}>{language==="it"?ch.labelIt:ch.labelEn}</span>
            </button>
          ))}
        </div>
        <div style={{display:"flex",gap:"16px",marginTop:"8px"}}>
          <button className="introBackBtn" onClick={()=>setScreen("approach")}>← {language==="it"?"Indietro":"Back"}</button>
          <button className="actionButton approachIntroCta" onClick={()=>setScreen("questIntro")}>{language==="it"?"Inizia dall'inizio":"Start from the beginning"} <b>→</b></button>
        </div>
      </section>
    </main>
  );
