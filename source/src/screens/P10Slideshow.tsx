...
export function P10Slideshow({language,setLanguage,setScreen,reset,p10SlideIdx,setP10SlideIdx,P10_SLIDES}:Props){
  return(
    <main style={{background:"#000",display:"grid",gridTemplateRows:"auto 1fr auto",height:"100dvh",overflow:"hidden"}}>
      <header style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"10px 20px",background:"rgba(0,0,0,.85)",zIndex:10}}>
        ...
        <span style={{color:"#aaa",fontSize:"13px"}}>{p10SlideIdx+1} / {P10_SLIDES.length}</span>
        <button className="langMini" onClick={()=>setScreen("blank1")} style={{background:"transparent",border:"1px solid #555",color:"#ccc",borderRadius:"4px",padding:"4px 10px",cursor:"pointer"}}>✕ {language==="it"?"Chiudi":"Close"}</button>
      </header>
      ...
      <footer style={{display:"flex",alignItems:"center",justifyContent:"center",gap:"24px",padding:"12px 20px",background:"rgba(0,0,0,.85)"}}>
        ...
        <div style={{display:"flex",gap:"6px"}}>
          {P10_SLIDES.map((_,i)=><span key={i} onClick={()=>setP10SlideIdx(i)} style={{width:"8px",height:"8px",borderRadius:"50%",background:i===p10SlideIdx?"#39efb4":"#555",cursor:"pointer",display:"inline-block"}}/>)}
        </div>
        <button onClick={()=>p10SlideIdx===P10_SLIDES.length-1?setScreen("blank1"):setP10SlideIdx(i=>i+1)} style={{background:"transparent",border:"none",cursor:"pointer"}}>
